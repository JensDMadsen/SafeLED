#include "ArduinoSafeLEDstats.h"

ArduinoSafeLEDstats led1(16,A0,15);             // WIRING: pinR --- resistor --- pinFb --- LED --- pinL
...
void setup() {
  ...
  led1.clearErrCount();                         // Clear error counter.
  
  if (led1.init(2)) {                           // Initialize and detect LED. Generate an error if mono LED.
...
void loop() {
  ...
  if ((int32_t)(millis() - nextMs) >= 0) {      // Check if toggle time has passed.
    ...
    Serial.print("Errors:   "); Serial.println(led1.errCount());
  }
}